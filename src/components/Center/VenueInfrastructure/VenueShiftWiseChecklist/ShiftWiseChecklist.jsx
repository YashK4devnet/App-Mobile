...
import styles from "./ShiftWiseChecklist.module.css";
import ChecklistForm from "./components/ChecklistForm";
import { getShiftWiseChecklist } from "./api/getShiftWiseChecklist";

const LOCAL_KEY = "ShiftWiseChecklistData";
const ShiftWiseChecklist = () => {
  ...
  const fetchChecklists = async () => {
    ...
    setError("");
    try {
      const response = await getShiftWiseChecklist(userLoginData);
      if (response?.Status) {
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.header}>
        <h1 className={styles.title}>Shift-Wise Checklist</h1>
        {!showFetchBtn && (
...
export default ShiftWiseChecklist;
