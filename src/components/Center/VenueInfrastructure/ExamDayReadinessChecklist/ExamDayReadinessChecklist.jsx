...
import styles from "./ExamDayReadinessChecklist.module.css";
import ChecklistForm from "./components/ChecklistForm";
import { getChecklist } from "./api/getChecklist";

const LOCAL_KEY = "ExamDayReadinessChecklistData";

const ExamDayReadinessChecklist = () => {
  ...
  const fetchChecklists = async () => {
    ...
    setError("");
    try {
      const response = await getChecklist(userLoginData);
      if (response?.Status) {
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.header}>
        <h1 className={styles.title}>Exam Day Readiness Checklist</h1>
        {!showFetchBtn && (
...
export default ExamDayReadinessChecklist;
