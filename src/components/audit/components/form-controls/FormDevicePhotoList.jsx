...
export function FormDevicePhotoList({
  label,
  name,
  value = [],
  error,
  ...
  const handleAddItem = () => {
    onChange(name, [...value, { deviceName: '', deviceImage: null }]);
  };
    ...
    <div className="space-y-4 p-5 bg-white border border-slate-200 rounded-2xl shadow-sm text-left">
      ...
      {value.length === 0 ? (
        <div className="text-center py-6 bg-slate-50 rounded-xl border border-dashed border-slate-300">
          <p className="text-[13px] text-slate-500 font-medium">No device photos added yet.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {value.map((item, index) => (
            <div key={index} className="relative p-4 bg-slate-50 border border-slate-200/60 rounded-xl animate-fade-in">
              ...
              <div className="space-y-4">
                <div>
                  <span className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mb-1 block">Device Name</span>
                  <input
                    type="text"
                    value={item.deviceName || ''}
                    onChange={(e) => handleItemChange(index, 'deviceName', e.target.value)}
                    placeholder="Enter device name (e.g. Core Switch, Firewall, Rack)"
                ...
                <div>
                  <FormImageUpload
                    ...
                    value={item.deviceImage}
                    onChange={(_, imgVal) => handleItemChange(index, 'deviceImage', imgVal)}
                  />
                </div>
              </div>
            </div>
