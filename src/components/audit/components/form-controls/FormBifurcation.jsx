      ...
      {lines.map((line, index) => (
        <div key={line.id ? `line-${line.id}` : `new-${index}`} className="flex items-center gap-3 bg-white/5 p-3 rounded-lg border border-white/10">
          ...
          <div className="flex-1">
            <label className="text-[10px] text-white/50 uppercase tracking-wider mb-1 block font-medium">Lab ID</label>
            <input
              type="number"
              value={line.labId || ''}
          ...
          {/* Floor ID */}
          <div className="flex-1">
            <label className="text-[10px] text-white/50 uppercase tracking-wider mb-1 block font-medium">Floor ID</label>
            <input
              type="number"
              value={line.floorId || ''}
