      ...
      >
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-black text-[#ff6b6b] uppercase tracking-wider bg-[#ff6b6b]/10 px-2 py-0.5 rounded">
      ...
      {isOpen && (
        <div className="relative z-40">
          <div 
            ...
            className="absolute top-0 left-0 right-0 bg-[#0F0F23] border-b border-white/20 shadow-2xl z-40 divide-y divide-white/10 animate-slide-down max-h-[45vh] sm:max-h-[calc(100vh-280px)] overflow-y-auto overscroll-contain [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:bg-[#ff6b6b]/50 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-white/5"
          >
              ...
              return (
                <button
                  ...
                  className={`w-full px-5 py-3.5 flex items-center justify-between text-[13px] transition-all hover:bg-white/10 cursor-pointer ${
                    isActive 
                      ? 'font-bold text-white bg-white/15 border-l-4 border-l-[#ff6b6b] pl-4' 
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {isCompleted && (
                      <span className="w-5 h-5 bg-[#4ecdc4] rounded-full flex items-center justify-center shadow-sm">
                        <CheckIcon className="w-3.5 h-3.5 text-[#0F0F23]" />
                      </span>
                    )}
                    {hasErrors && (
                      <span className="w-5 h-5 bg-[#ff6b6b] rounded-full flex items-center justify-center shadow-sm">
                        <ExclamationCircleIcon className="w-3.5 h-3.5 text-white" />
                      </span>
                    )}
                    {isActive && (
                      <span className="text-[10px] uppercase font-black tracking-wider text-[#ff6b6b] bg-[#ff6b6b]/20 px-2 py-0.5 rounded border border-[#ff6b6b]/30">
                        Active
                      </span>
                    )}
                  </div>
