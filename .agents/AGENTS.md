...
- **Background Live Tracking Integration & Native HTTP Bypass:** 
  ...
  - **Native HTTP POST:** The Java receiver spawns a background thread and sends the HTTP POST directly to `/api/employee/location/log` using `HttpURLConnection`, natively bypassing the frozen JavaScript environment while preventing battery-draining CPU WakeLocks.
  - **Config Sync:** JavaScript (`LiveTracking.jsx`) securely passes the `serverApiKey`, `employeeId`, and `baseUrl` down to the `NativeTrackingPlugin` before starting tracking, keeping the native layer synced with the React auth state.
